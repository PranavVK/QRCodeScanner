...
class EventDetailsSelectionPage extends Component {
  ...
  constructor(props) {
        ...
        this.state = {
            ...
            sessionDetails: [{'title': "Need of Ethereum", 'timing': "12th November, 9 AM - 4 PM", 'description': "Importance of new system and User Types of Ethereum"},{'title': "Study of Ethereum", 'timing': "13th November, 9 AM - 4 PM", 'description': "Ethereum Foundation and Analysis of White and Yell..."}],
            isExpanded: false,
    ...
    _renderItem = ({ item, index }) => (
      <TouchableOpacity onPress={() => {}}>
        <View style={{ flexDirection: 'column', justifyContent: "flex-start", alignItems: "flex-start", width: deviceWidth}}>
              <Text style={{fontSize: 20, fontWeight: '500', paddingLeft: 10, marginTop: 8 }}>{item.title}</Text>
              ...
              <Image style={{height: 20, width:20}} source={require('../Assets/Icons/clock-with-white-face.png')}/>
                ...
                <Text style={{fontSize: 16, fontWeight: "normal", marginLeft: 10, marginTop: 2}}>{item.timing}</Text>
              </View>
              <Text style={{fontSize: 14, fontWeight: '200', paddingLeft: 10, marginTop: 4, marginBottom: 8}}>{item.description}</Text>
          </View>
        ...
        <ScrollView style={{backgroundColor: STYLE_CONSTANTS.BACKGROUND_COLOR, marginBottom: 10}}>
          ...
          <TouchableOpacity onPress={() => {
            ...
            <View style={{flex: 1, flexDirection: 'column', width: deviceWidth}}>
              <Text style={{paddingLeft: 10, paddingRight: 10,fontSize: 25, fontWeight: '600', marginTop: 8}}>{this.state.item.title}</Text>
              {(!this.state.isTextViewCollapsed || this.state.isExpanded) && <View style={{paddingLeft: 10, paddingRight: 10, marginTop: 8}}>
                <Text style={{fontSize: 20, fontWeight: '300'}}onLayout={( event ) => {
